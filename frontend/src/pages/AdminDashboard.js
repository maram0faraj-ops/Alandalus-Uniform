 ...
 useEffect(() => {
  const fetchDashboardData = async () => {
   ...
   try {
    ...
    setStatusChartData({
     labels: statusStatsRes.data.map(item => item._id === 'in_stock' ? 'في المخزون' : (item._id === 'new' ? 'جديد' : 'تم التسليم')),
     datasets: [{
      data: statusStatsRes.data.map(item => item.count),
      backgroundColor: ['#36a2eb', '#ffce56', '#ff6384'], // Blue, Yellow, Pink
     }, ],
    });
    ...
    <Col lg={7} className="mb-4">
     <Card className="shadow-sm h-100">
      <Card.Body>
       ...
       {stageChartData && <Bar data={stageChartData} options={{ responsive: true }} />}
      </Card.Body>
     </Card>
    </Col>
