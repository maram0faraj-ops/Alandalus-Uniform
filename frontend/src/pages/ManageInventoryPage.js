...
function ManageInventoryPage() {
    ...
    // استخدام المتغيرات التي تسببت في الخطأ
    ...
    const handleScanSuccess = (decodedText) => {
        ...
        const item = allItems.find(i => i.barcode === decodedText);
        if (item) {
            setItemsToDelete([item]);
            setShowConfirmModal(true);
        } else {
            alert("الباركود غير موجود في المخزون");
        }
    };
    ...
    try {
        ...
        // تحضير مصفوفة المعرفات
        const idsArray = itemsToDelete.map(item => item._id);
        
        // إرسال طلب حذف واحد لكل العناصر
        await api.delete('/api/inventory/bulk-delete', { data: { ids: idsArray } });
        
        // تحديث الواجهة محلياً بعد نجاح الحذف
        const deletedIdsSet = new Set(idsArray);
        setAllItems(currentItems => currentItems.filter(item => !deletedIdsSet.has(item._id)));
        
        setShowConfirmModal(false);
        setSelectedIds(new Set());
        setItemsToDelete([]);
        alert('تم حذف العناصر المحددة بنجاح.');
    } catch (err) {
        console.error("Deletion Error:", err);
        setError(err.response?.data?.msg || 'فشل في حذف العناصر المحددة. يرجى المحاولة مرة أخرى.');
    } finally {
        setLoading(false);
    }
};
    ...
    return (
        <Container className="mt-5 text-end" dir="rtl">
            ...
            {/* استخدام مكون الماسح الضوئي لتجاوز خطأ ESLint */}
            {showScanner && (
                <Modal show={showScanner} onHide={() => setShowScanner(false)} centered>
                    <Modal.Header closeButton><Modal.Title>امسح QR للحذف</Modal.Title></Modal.Header>
                    <Modal.Body>
            ...
            <Card className="mb-4 shadow-sm">
                ...
                <Card.Body>
                    <Row>
                        ...
                        {/* إضافة فلترة المقاس */}
                        <Col md={3}><Form.Select value={filters.size} onChange={(e)=>setFilters({...filters, size: e.target.value})}><option value="all">كل المقاسات</option>{filterOptions.sizes.map(z => <option key={z} value={z}>{z}</option>)}</Form.Select></Col>
                        <Col md={3}><Button variant="secondary" className="w-100" onClick={() => setFilters({stage:'all', type:'all', size:'all'})}>إعادة تعيين</Button></Col>
                    </Row>
                </Card.Body>
            </Card>
            ...
            <Table striped bordered hover responsive className="text-center align-middle">
                <thead className="bg-dark text-white">
                    <tr>
                        ...
                        <th>#</th><th>المرحلة</th><th>النوع</th><th>المقاس</th><th>الباركود</th><th>تاريخ الإضافة</th><th>الإجراء</th>
                    </tr>
                </thead>
                <tbody>
            ...
            <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)} centered>
                ...
                <Modal.Footer><Button variant="secondary" onClick={() => setShowConfirmModal(false)}>إلغاء</Button><Button variant="danger" onClick={handleDeleteConfirmed}>تأكيد الحذف</Button></Modal.Footer>
            </Modal>
        </Container>
    );
}
