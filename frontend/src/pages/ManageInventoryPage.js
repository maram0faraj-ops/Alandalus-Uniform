...
function ManageInventoryPage() {
    ...
    const fetchItems = useCallback(async () => {
        try {
            ...
            // استخراج خيارات الفلترة المتاحة فعلياً في المخزون
    ...
    // تنفيذ الفلترة عند تغيير أي خيار
    useEffect(() => {
        let result = allItems;
    ...
    return (
        <Container className="mt-5 text-end" dir="rtl">
            ...
            <Card className="mb-4 shadow-sm">
                ...
                <Card.Body>
                    <Row>
                        <Col md={3}><Form.Select value={filters.stage} onChange={(e)=>setFilters({...filters, stage: e.target.value})}><option value="all">كل المراحل</option>{filterOptions.stages.map(s => <option key={s} value={s}>{s}</option>)}</Form.Select></Col>
                        <Col md={3}><Form.Select value={filters.type} onChange={(e)=>setFilters({...filters, type: e.target.value})}><option value="all">كل الأنواع</option>{filterOptions.types.map(t => <option key={t} value={t}>{t}</option>)}</Form.Select></Col>
                        {/* عمود فلترة المقاس المضاف */}
